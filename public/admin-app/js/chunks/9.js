    ...
    loadContent: function loadContent() {
      ...
      this.$http.get(action).then(function (response) {
        ...
        for (var i = 0; i < _this3.rooms.length; i++) {
          _this3.roomOptions.push({
            value: _this3.rooms[i].id,
            label: _this3.rooms[i].name
          });
        }

        _this3.users = res.users;

        for (var _i = 0; _i < _this3.users.length; _i++) {
          _this3.userOptions.push({
            value: _this3.users[_i].id,
            label: "".concat(_this3.users[_i].first_name, " ").concat(_this3.users[_i].last_name)
          });
        }
